...
- **Resolve manually** — opens a **conflict-marked copy** of the target in your editor (`$VISUAL`/`$EDITOR`): at the drifted spot you see, git-conflict style, what the file currently has (`-[current]`) versus what the patch proposes (`-[patch proposes]`, annotated with the line it expected to find), so you know exactly *what* to change and *where*. Pick or edit the lines you want, **delete the markers**, and save — ai-playbook writes the reconciled result back to the real file and re-checks the patch; if the change now applies, the drift region clears. Use this when the correct new value depends on business logic you need to reason about — here, deciding whether `60` or some other value is right given that someone deliberately set it to `99`. If you leave the markers in place the edit counts as **unresolved** and the file is left unchanged. (If ai-playbook can't locate the drifted region to mark it, it falls back to opening the raw target file instead.)
