...
func FinalPlaybookPrompt(req capture.Request, base, context string) string {
	...
	if amend {
		...
		b.WriteString("However, if the change note rejects the current approach outright, discard the base playbook and re-author from scratch honoring the note — do not patch the rejected approach.\n\n")
