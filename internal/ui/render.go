...
type renderer struct {
	...
	buttons []Button
	blocks  []Block
	// parsed is the canonical block list from playbook.ParseBlocks — the SINGLE
	// schema owner (ADR-0009 step 1). code() consumes it in document order via
	// parsedIdx instead of re-deriving block semantics from the fence info, so the
	// renderer and the parser can never disagree.
	parsed    []Block
	parsedIdx int
...
func (r *renderer) code(n ast.Node) {
	...
	// Blocks are owned by playbook.ParseBlocks (ADR-0009 step 1): it walks the SAME
	// normalized document top-level-only, so r.parsed enumerates exactly the fenced/
	// indented code nodes code() is invoked for, in the same order. Consume the next
	// one rather than re-deriving lang/id/needs/type here, so the renderer and the
	// parser can never disagree. blk.Payload equals src by construction.
	blk := r.parsed[r.parsedIdx]
	r.parsedIdx++
...
// countBlocks returns the number of code blocks Render would enumerate into its
// Block list — every top-level fenced or indented code block — WITHOUT running the
// full styled render. It is derived directly from the single schema owner,
// playbook.ParseBlocks (ADR-0009 step 1), so its count can never disagree with
// Render's block list. Used by the block-count validity checks (isValidPlaybook via
// ValidatePlaybook and commitPlaybookCmd). TestCountBlocksMatchesRender pins the
// equality across a fixture corpus; the delegation is what keeps it true.
