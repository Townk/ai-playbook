...
func (shAdapter) job(p jobParams) string {
	...
	// >|/2>| force the capture redirect through `set -C` (noclobber) — on a re-run
	// the session-dir target already exists, and a plain `>` would fail under
	// noclobber (leaving the stale first-run bytes). >| is POSIX and verified in
	// dash; matches the cwd trap's >|.
