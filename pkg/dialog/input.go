// Package dialog is the interaction toolkit: the themed terminal dialog
// widgets (confirm/line/text/choose/form) shared by ai-playbook's own viewer
// and floats, the hidden `ai-playbook input` subcommand, and the standalone
// `ask` binary. Widget behavior is an external contract — `ask`'s documented
// CLI (subcommand-per-widget, exit codes as the answer, ASK_* env theming,
// JSON form specs) pins it — so changes here must consider that surface, not
// just ai-playbook's own dialogs.
//
