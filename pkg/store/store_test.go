...
func TestIndexNewestFirstAndProjPrefix(t *testing.T) {
	globalDir := t.TempDir()
	projDir := t.TempDir()
	d := Dirs{Global: globalDir, Project: projDir}
...
func TestSearchSubstringCaseInsensitive(t *testing.T) {
	...
	if got := d.Search("deploy"); len(got) != 1 || got[0].Slug != "alpha" {
		t.Errorf("search deploy = %+v", got)
	}
	// category match
	if got := d.Search("BUILD"); len(got) != 1 || got[0].Slug != "proj:beta" {
		t.Errorf("search BUILD = %+v", got)
	}
	// tag match
	if got := d.Search("kubernetes"); len(got) != 1 || got[0].Slug != "alpha" {
		t.Errorf("search kubernetes = %+v", got)
	}
	// description match
	if got := d.Search("compile"); len(got) != 1 || got[0].Slug != "proj:beta" {
...
func TestPathForUnknownAndKnown(t *testing.T) {
	...
	if _, ok := d.PathFor("nope"); ok {
		t.Errorf("PathFor(nope) ok=true, want false")
	}
	got, ok := d.PathFor("alpha")
	if !ok || got != want {
...
func TestLoad_ReadsProjectBound(t *testing.T) {
	globalDir := t.TempDir()
	d := Dirs{Global: globalDir, Project: t.TempDir()}
