...
func FollowupPrompt(req capture.Request, failedOutput string) string {
	...
	b.WriteString("  2. A fresh fix block with a new approach.\n")
	b.WriteString("  3. A SEPARATE verify block tagged {id=verify needs=<fix-id>} that re-runs\n")
...
func WrapupPrompt(req capture.Request, runlog string) string {
	...
	b.WriteString("(2) Write a short `## Solution` section a teammate could reuse later.\n")
	b.WriteString("(3) If the request appears resolved, call `ai-assist-remember` exactly once\n")
	b.WriteString("    with one durable, reusable fact about this project — something that would\n")
	b.WriteString("    help a future session. Example:\n")
