...
- [ ] cursor FULL: WORKSPACE-config route is blocked (Phase B, cursor-agent 2026.07.01-777f564 — project `.cursor/mcp.json` MERGES with the user's global config, no `--strict-mcp-config`, blanket `--approve-mcps` only). NOT YET TRIED: a config-DIR REDIRECT (`CURSOR_CONFIG_DIR`/`CURSOR_HOME`/`HOME` override → a pristine config root) that isolates cursor from `~/.cursor/mcp.json` entirely — **run Phase C of `docs/specifications/cursor-full-promotion.md` on the laptop** (deterministic `mcp list` isolation oracle + auth-survival + approval + schema-enforcement probes; ~1 LLM call). If a redirect isolates cleanly and preserves auth, promote to FULL (or partial FULL) behind a runtime isolation guard; else cursor stays BASIC (2026-07-06)
...
- [ ] Confirm-yes/no and assisted-footer Screen buttons get hint labels but never painted chips — same gap U1 closed for the header pills (2026-07-06)
