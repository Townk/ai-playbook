...
- [ ] cursor FULL is BLOCKED on MCP isolation (Phase A done + live-verified 2026-07-06; Phase B probed on cursor-agent 2026.07.01-777f564). cursor-agent MERGES project `.cursor/mcp.json` with the user's global `~/.cursor/mcp.json` (no `--strict-mcp-config` analog) and only offers blanket `--approve-mcps`, so attaching our tools leaks the user's global servers (atlassian/zellij/context7) into the authoring session — a leaky FULL is worse than BASIC, so cursor stays BASIC. Revisit ONLY if cursor-agent adds strict/isolated MCP config loading + scoped headless approval (evidence in `docs/specifications/cursor-full-promotion.md`) (2026-07-06)
