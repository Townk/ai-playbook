...
- [ ] Coverage residual: total 84.7% (2026-07-19 — the old per-package figures were stale: mcpserver 83%, capture 87%, triage 100%, floatinput 97%). The remaining gap to ~90% is almost entirely the integration entry-point surface (cli finalize/mcp/selftest 46%, launcher session/assist/answer TUIs 76%, dialog Run* runners) — covered by the Ideas E2E/PTY item, not by more unit tests (2026-07-19)
...
- [ ] (low priority) E2E/integration tests for the integration entry points (`launcher` entry points, `cmd` `selftest`/`mcpMain`) — spawn the real binary + drive a TUI/PTY. These render via live mux/model/TUI/driver so they're not unit-testable; coverage there is intentionally low. Would close most of the measured 84.7%→~90% gap (2026-06-27, figures refreshed 2026-07-19)
