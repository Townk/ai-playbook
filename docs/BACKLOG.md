...
- [ ] Promote the cursor harness to FULL — **run on the work-laptop (has `cursor-agent`)** following the detailed two-phase brief `docs/specifications/cursor-full-promotion.md` (Phase A: verify the doc-derived BASIC adapter against the real CLI, replace fixtures with captures; Phase B: gate FULL on an ISOLATED per-invocation MCP transport + schema-enforcing tool loop + scoped headless approval — stay BASIC if isolation can't be proven, a leaky FULL is worse). Backlog line stays until FULL ships or the brief narrows the blocker (2026-07-06)
