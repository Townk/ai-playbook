...
- [ ] Promote the cursor harness to FULL: strongest candidate transport is `--workspace <temp dir>` holding OUR OWN `.cursor/mcp.json` (a root we control — neutralizes the mutates-user-file and races-concurrent-sessions objections); remaining blockers — the user's global `~/.cursor/mcp.json` servers still load (no `--strict-mcp-config` analog), headless MCP approval is undocumented (`--approve-mcps` blanket-approves the user's servers too), and no doc establishes a schema-enforcing tool re-ask loop (cursor.com/docs/cli/mcp). Needs live probes on a machine with the CLI; the RequireHarness-gated live tests are the gate, and the shared mcpServers-document writer factoring (spec) rides the promotion (2026-07-06)
